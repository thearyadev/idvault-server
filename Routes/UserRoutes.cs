  ...
  public static void MapUserRoutes(this IEndpointRouteBuilder endpoints) {
    ...
    endpoints.MapGet("/users/me", async context => {
      ...
      if (user == null) {
        ...
        await context.Response.WriteAsJsonAsync(new { error = new {
    ...
    endpoints.MapPost("/users/login", async context => {
      ...
      var user = dbContext!.Users.FirstOrDefault(
          u => u.Username == username.ToString() &&
      ...
      var tokenString =
          JwtTokenValidator.GenerateTokenWithUserName(user!.Username);
    ...
    endpoints.MapPost("/users/register", async context => {
      var form_data = await context.Request.ReadFormAsync();
      ...
      if (fullname == null || email == null || phone_number == null ||
      ...
      var new_user = new User {
        Name = fullname,     Email = email,       PhoneNumber = phone_number,
